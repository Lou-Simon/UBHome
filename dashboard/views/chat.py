...
def delete_email_view(request):
    ...
    current_box = request.POST.get('current_box', 'inbox')

    if email_id:
        try:
            email = Email.objects.get(id=email_id)
            # Si c'est un brouillon, suppression définitive
            if email.is_draft and email.sender == student:
                email.delete()
            # Si c'est un email reçu
...
def chat_view(request):
    ...
    if request.method == 'POST':
        ...
        # 1. SAUVEGARDE AUTOMATIQUE (Appelée par JS à la fermeture)
        if request.POST.get('action') == 'autosave_draft':
            draft_id = request.POST.get('draft_id')
            subject = request.POST.get('subject', '')
            body = request.POST.get('body', '')

            # On ne sauvegarde pas si tout est vide
            ...
            # Note: recipient est None pour un brouillon générique
            email = Email.objects.create(
                sender=student, recipient=None, subject=subject, body=body, is_draft=True
            )
        ...
        if 'send_email' in request.POST:
            if form.is_valid():
                ...
                draft_id = request.POST.get('draft_id') # ID du brouillon à supprimer
                ...
                if not recipients:
                    ...
                    for target in recipients:
                        Email.objects.create(
                            sender=student, recipient=target, subject=subject, body=body, is_draft=False
                        )

                    # Suppression du brouillon maintenant qu'il est envoyé
                    if draft_id:
                        try:
    ...
    context = {
        ...
        'unread_count': unread_count, 'draft_count': draft_count, 'total_email_notifs': total_email_notifs
    }
